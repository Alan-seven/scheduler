	...
	private void saveSumq(DateInterval dataInterval){
		...
		switch (dataInterval){
			case HOUR:
				...
				stationFlowSumService.saveSumq(flowSums,dataInterval.getType()+"","1");
				break;
			case DAY:
			...
			case FIVEDAYS:
				...
				for(FlowSum flowSum:flowSums){
					if("90210530".equals(flowSum.getStcd())){
						continue;
					}
	...
	private void saveXierheSumq(DateInterval dataInterval){
		...
		switch (dataInterval){
			case HOUR:
				...
				stationFlowSumService.saveSumq(flowSums,dataInterval.getType()+"","2");
				break;
			case DAY:
